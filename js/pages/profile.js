        ...
        <article class="thinktank-card">
          ...
          <form id="passwordForm">
            <label class="auth-field-label">
              ...
              <div class="password-field-wrap" style="position:relative;">
                <input type="password" id="profileCurrentPassword" class="thinktank-input" autocomplete="current-password" style="padding-right:40px;">
                <button type="button" class="toggle-password-btn" data-target="profileCurrentPassword" aria-label="${getText("profile.showPassword", "Εμφάνιση κωδικού")}" style="position:absolute; right:8px; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; font-size:1.1rem; line-height:1;">👁</button>
              </div>
            </label>
            ...
            <label class="auth-field-label">
              ...
              <div class="password-field-wrap" style="position:relative;">
                <input type="password" id="profileNewPassword" class="thinktank-input" autocomplete="new-password" style="padding-right:40px;">
                <button type="button" class="toggle-password-btn" data-target="profileNewPassword" aria-label="${getText("profile.showPassword", "Εμφάνιση κωδικού")}" style="position:absolute; right:8px; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; font-size:1.1rem; line-height:1;">👁</button>
              </div>
            </label>
            ...
            <label class="auth-field-label">
              ...
              <div class="password-field-wrap" style="position:relative;">
                <input type="password" id="profileConfirmNewPassword" class="thinktank-input" autocomplete="new-password" style="padding-right:40px;">
                <button type="button" class="toggle-password-btn" data-target="profileConfirmNewPassword" aria-label="${getText("profile.showPassword", "Εμφάνιση κωδικού")}" style="position:absolute; right:8px; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; font-size:1.1rem; line-height:1;">👁</button>
              </div>
            </label>
...
export async function afterRender() {
  ...
  // Show/hide toggle for password fields — lets the member see what
  // they're typing rather than only masked dots.
  document.querySelectorAll(".toggle-password-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const targetInput = document.getElementById(btn.dataset.target);
      if (!targetInput) return;

      const isShowing = targetInput.type === "text";
      targetInput.type = isShowing ? "password" : "text";
      btn.textContent = isShowing ? "👁" : "🙈";
      btn.setAttribute(
        "aria-label",
        isShowing
          ? getText("profile.showPassword", "Εμφάνιση κωδικού")
          : getText("profile.hidePassword", "Απόκρυψη κωδικού")
      );
    });
  });
