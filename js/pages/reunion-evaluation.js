import { createReunionEvaluation } from "../api/evaluationApi.js";
import { getText } from "../services/textService.js";

const REUNION_RATING_FIELDS = [
...
export async function render() {
  return getText("evaluation.renderHtml", `
    <section class="evaluation-page">
      ...
      <h2>Αξιολόγηση Reunion 50</h2>

      <p>
        Η γνώμη σας μας βοηθά να βελτιώσουμε τις εκδηλώσεις της κοινότητας.
      ...
      <form id="reunionEvaluationForm" class="event-registration-form evaluation-form">
        ...
        <fieldset class="evaluation-group">
          ...
          ${REUNION_RATING_FIELDS.map(starRatingMarkup).join("")}
        </fieldset>
...
export async function afterRender() {
  ...
  form.addEventListener("submit", async event => {
    ...
    const missingRating = REUNION_RATING_FIELDS.find(field => !ratingValues[field.id]);
    ...
    try {
      ...
      statusMessage.innerHTML =
        getText("evaluation.successHtml", "✓ Ευχαριστούμε! Η αξιολόγηση της εκδήλωσης καταχωρήθηκε με επιτυχία.");
