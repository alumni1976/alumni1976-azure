import { createWebsiteEvaluation } from "../api/websiteEvaluationApi.js";
...
export async function render() {
  return getText("websiteevaluation.renderHtml", `
    <section class="evaluation-page">
      <p class="section-tag">Αξιολόγηση</p>
      <h2>Αξιολόγηση Ιστοσελίδας</h2>

      <p>
        Η γνώμη σας μας βοηθά να βελτιώνουμε συνεχώς την ιστοσελίδα. Η συμπλήρωση
        διαρκεί περίπου 1 λεπτό.
      </p>

      <form id="websiteEvaluationForm" class="event-registration-form evaluation-form">
...
export async function afterRender() {
  const form = document.getElementById("websiteEvaluationForm");
  ...
  form.addEventListener("submit", async event => {
    ...
    if (missingRating) {
      statusMessage.textContent =
        getText("websiteevaluation.ratingRequired", `Παρακαλώ βαθμολογήστε: "${missingRating.label}".`);
      return;
    }

    if (!consentInput.checked) {
      statusMessage.textContent =
        getText("websiteevaluation.consentRequired", "Παρακαλώ αποδεχτείτε την αποθήκευση στοιχείων για να συνεχίσετε.");
      return;
    }
    ...
    statusMessage.textContent = getText("websiteevaluation.saving", "Αποθήκευση αξιολόγησης...");
    ...
    try {
      ...
      statusMessage.innerHTML =
        getText("websiteevaluation.successHtml", "✓ Ευχαριστούμε! Η αξιολόγησή σας καταχωρήθηκε με επιτυχία.");

    } catch (err) {
      console.error("Error saving website evaluation:", err);

      statusMessage.textContent =
        err?.message || getText("websiteevaluation.saveError", "Αποτυχία αποθήκευσης αξιολόγησης. Δοκιμάστε ξανά.");
